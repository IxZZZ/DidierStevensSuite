...
def ProcessBinaryFile(filename, content, dSummary, cutexpression, flag, oOutput, oLogfile, options, oParserFlag):
    if content == None:
        try:
...
def ProcessBinaryFiles(filenames, oLogfile, options, oParserFlag):
    ...
    if options.jsoninput:
        ...
        for item in items:
            ...
            index += 1
            ProcessBinaryFile(item['name'], item['content'], dSummary, '', '', oOutput, oLogfile, options, oParserFlag)
    else:
        for filename, cutexpression, flag in filenames:
            oOutput.Filename(filename, index, len(filenames))
            index += 1
            ProcessBinaryFile(filename, None, dSummary, cutexpression, flag, oOutput, oLogfile, options, oParserFlag)

    oOutput.Line('Summary:')
    keysFound = dSummary.get('keys', [])
    if len(keysFound) == 0:
        oOutput.Line('No keys found')
    else:
        oOutput.Line('Keys found:')
        for key in keysFound:
            oOutput.Line(key)

    filesWritten = dSummary.get('files', [])
    if len(filesWritten) == 0:
        oOutput.Line('No files written')
    else:
        oOutput.Line('Files written:')
