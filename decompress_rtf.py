...
__description__ = 'Tool to decompress compressed RTF'
...
This is a tool to decompress compressed RTF.
...
def FilenameCheckHash(filename, literalfilename):
    if literalfilename:
        ...
        return (FCH_ERROR, 'expression') if result is None else (FCH_DATA, result)
    ...
    def __init__(self, filenames, literalfilenames=False, recursedir=False, checkfilenames=False, expressionprefix=None):
        ...
        if len(filenames) == 0:
            ...
            for dirwildcard in filenames:
                ...
                elif dirwildcard.startswith('@'):
                    self.filenameexpressions.extend(
                        [filename, expression]
            ...
            self.warning = (
                ...
                and not self.filenameexpressions
            )
            ...
            if self.warning:
                ...
                return
        if not self.filenameexpressions and expression != '':
...
def Quote(value, separator, quote):
    ...
    return quote + value + quote if separator in value or value == '' else value
    ...
    def __init__(self, keyword, comment):
        ...
        if keyword == '':
            ...
            self.oOutput = cOutput(
                f'{os.path.splitext(os.path.basename(sys.argv[0]))[0]}-{keyword}-{self.FormatTime()}.log'
            )

        self.Line('Start')
        self.Line('UTC', '%04d%02d%02d-%02d%02d%02d' % time.gmtime(time.time())[:6])
...
def ProcessBinaryFile(filename, content, cutexpression, oOutput, oLogfile, options):
    if content == None:
        try:
...
def ProcessBinaryFiles(filenames, oLogfile, options):
    ...
    if options.jsoninput:
        ...
        for filename, cutexpression in filenames:
            ...
            index += 1
            ProcessBinaryFile(filename, None, cutexpression, oOutput, oLogfile, options)
